...
**0.2.1은 실제 아이폰에서 한 장의 파일명·촬영일시·크기 확인 → 재생성 → PNG 2개와 JPEG 1개 저장 → 원본 복원까지 통과했습니다.** JPEG는 2592 × 1936 픽셀로 전체 디코딩·바이트 수·SHA-256을 검증했고, 복원 후 같은 asset의 원본/현재 이미지 해시가 실행 전과 같고 편집이 없음을 다시 확인했습니다. 보조 앱의 실기기 서명 빌드·설치와 오프라인 테스트 67개도 통과했습니다. 전체 앨범 JPEG 처리와 강제 중단 후 실기기 복구는 아직 검증하지 않았습니다.
...
**JPEG 모드**에서는 위 5번부터 **Save → 필요하면 완료 → JPEG 내보내기 → Mac에서 이미지·크기·SHA-256 확인 → PhotoKit으로 같은 사진 원본 복원 → 복원 검증 → 다음 사진** 순서로 진행합니다. JPEG는 최대 크기 편집 결과를 8비트 sRGB/품질 1.0으로 재인코딩하며 원본 HEIC/RAW의 바이트나 HDR·Live Photo 동영상은 보존하지 않습니다. 기존 편집이 있는 사진은 건너뛰고 기록합니다. 정보 패널의 촬영일시(기기 현지 시각, 분 단위)와 가로·세로 크기를 읽을 수 있으면 원래 파일명과 함께 정확히 대조합니다. 모든 조건에 맞는 사진이 여러 장이면 임의로 고르지 않고 중단합니다. 날짜·크기를 읽지 못하면 파일명 자체가 고유해야 합니다.
...
```text
dist/
  iphone-photos-automation-0.2.1.zip
  iphone-photos-automation-0.2.1.zip.sha256
```
